...
Elm.Counter.make = function (_elm) {
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var storageInput = Elm.Native.Port.make(_elm).inboundSignal("storageInput",
   "Counter.Model",
   function (v) {
      return typeof v === "object" && "count" in v && "increment" in v && "decrement" in v ? {_: {}
                                                                                             ,count: typeof v.count === "number" && isFinite(v.count) && Math.floor(v.count) === v.count ? v.count : _U.badPort("an integer",
                                                                                             v.count)
                                                                                             ,increment: typeof v.increment === "number" && isFinite(v.increment) && Math.floor(v.increment) === v.increment ? v.increment : _U.badPort("an integer",
                                                                                             v.increment)
                                                                                             ,decrement: typeof v.decrement === "number" && isFinite(v.decrement) && Math.floor(v.decrement) === v.decrement ? v.decrement : _U.badPort("an integer",
                                                                                             v.decrement)} : _U.badPort("an object with fields `count`, `increment`, `decrement`",
      v);
   });
   var jsActions = Elm.Native.Port.make(_elm).inboundSignal("jsActions",
   "Int",
   function (v) {
   ...
   var increment = Elm.Native.Port.make(_elm).outboundSignal("increment",function (v) {    return [];},incrementMailbox.signal);
   var initialModel = {count: 0,increment: 0,decrement: 0};
   var storageMailbox = $Signal.mailbox(initialModel);
   var storage = Elm.Native.Port.make(_elm).outboundSignal("storage",
   function (v) {
      return {count: v.count,increment: v.increment,decrement: v.decrement};
   },
   storageMailbox.signal);
   var NoOp = {ctor: "NoOp"};
   var sendToIncrementMailbox = A2($Effects.map,$Basics.always(NoOp),$Effects.task(A2($Signal.send,incrementMailbox.address,{ctor: "_Tuple0"})));
   var sendToStorageMailbox = function (model) {    return A2($Effects.map,$Basics.always(NoOp),$Effects.task(A2($Signal.send,storageMailbox.address,model)));};
   var update = F2(function (action,model) {
      var _p0 = action;
      switch (_p0.ctor)
      {case "Increment": var newModel = _U.update(model,{count: model.count + 1,increment: model.increment + 1});
           return {ctor: "_Tuple2",_0: newModel,_1: $Effects.batch(_U.list([sendToIncrementMailbox,sendToStorageMailbox(newModel)]))};
         case "Decrement": var newModel = _U.update(model,{count: model.count - 1,decrement: model.decrement + 1});
           return {ctor: "_Tuple2",_0: newModel,_1: sendToStorageMailbox(newModel)};
         case "SetModel": return {ctor: "_Tuple2",_0: _p0._0,_1: $Effects.none};
         default: return {ctor: "_Tuple2",_0: model,_1: $Effects.none};}
   });
   var SetModel = function (a) {    return {ctor: "SetModel",_0: a};};
   var mapStorageInput = function (model) {    return SetModel(model);};
   ...
   var view = F2(function (address,model) {
      ...
      _U.list([A2($Html.button,_U.list([A2($Html$Events.onClick,address,Decrement)]),_U.list([$Html.text("-")]))
              ...
              ,A2($Html.h3,
              _U.list([]),
              _U.list([$Html.text(A2($Basics._op["++"],"- clicked ",A2($Basics._op["++"],$Basics.toString(model.decrement)," times")))]))
              ,A2($Html.h3,
              _U.list([]),
              _U.list([$Html.text(A2($Basics._op["++"],"+ clicked ",A2($Basics._op["++"],$Basics.toString(model.increment)," times")))]))]));
   });
   var mapJsAction = function ($int) {    var _p1 = $int;if (_p1 === 1) {    return Increment;} else {    return NoOp;}};
